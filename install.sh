...
if command -v termux-setup-storage; then
  printf "${RED}For termux, please use https://raw.githubusercontent.com/The-KurupTg-project/KurupUserbot/main/termux-install.sh${NC}\n" # skipcq
  exit 1
fi
...
if [[ -d "KurupUserbot" && "$(basename "$PWD")" != "KurupUserbot" ]]; then
  cd KurupUserbot || exit 2
elif [[ "$(basename "$PWD")" == "KurupUserbot" && -f ".env.dist" && -f "main.py" && -d "modules" ]]; then
  printf "${BLUE}Already inside the KurupUserbot repo, proceeding...${NC}\n" # skipcq
else
  git clone https://github.com/The-KurupTg-project/KurupUserbot || exit 2
  cd KurupUserbot || exit 2
fi

if [[ -f ".env" ]] && [[ -f "my_account.session" ]]; then
  printf "${GREEN}It seems that KurupUserbot is already installed. Exiting...${NC}\n" # skipcq
  exit
fi
...
# Adjust the ownership of the KurupUserbot directory
...
while true; do
  ...
  1)
    ...
    su -c "pm2 start main.py --name Kurup --interpreter python3" $SUDO_USER
    su -c "pm2 save" $SUDO_USER

    printf "${GREEN}============================\\n" # skipcq
    printf "Great! KurupUserbot installed successfully and running now!\n"
    printf "Installation type: PM2\n"
    printf "Start with: \"pm2 start Kurup\"\n"
    printf "Stop with: \"pm2 stop Kurup\"\n"
    printf "Process name: Kurup\n"
    printf "============================${NC}\n" # skipcq
    break
    ;;
  2)
    cat >/etc/systemd/system/Kurup.service <<EOL
[Unit]
Description=Service for Kurup Userbot
[Service]
Type=simple
...
EOL
    systemctl daemon-reload
    systemctl start Kurup
    systemctl enable Kurup

    printf "${GREEN}============================\\n" # skipcq
    printf "Great! KurupUserbot installed successfully and running now!\n"
    printf "Installation type: Systemd service\n"
    printf "Start with: \"sudo systemctl start Kurup\"\n"
    printf "Stop with: \"sudo systemctl stop Kurup\"\n"
    printf "============================${NC}\n" # skipcq
    break
    ;;
  3)
    printf "${GREEN}============================\\n" # skipcq
    printf "Great! KurupUserbot installed successfully!\n"
...
# Adjust the ownership of the KurupUserbot directory again as a final step
chown -R $SUDO_USER:$SUDO_USER .
