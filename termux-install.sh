...
if [[ -d "KurupUserbot" ]]; then
  cd KurupUserbot || exit
...
if [[ -f ".env" ]] && [[ -f "my_account.session" ]]; then
  echo "It seems that KurupUserbot is already installed. Exiting..."
  exit
fi
...
echo
echo "============================"
echo "Great! KurupUserbot installed successfully!"
echo "Start with: \"cd KurupUserbot && python3 main.py\""
echo "============================"
